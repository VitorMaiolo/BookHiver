...
from emprestimos import registrarEmprestimo
...
def leitores():
    ...
    try:
        csr.execute(
            "SELECT nome, email, telefone, id, cpf FROM leitores ORDER BY nome"
        )
...
def logarUsuario():
    dados = login(request)
    if dados.get('logado') == True:
        return render_template("main-page.html", 
                               acervo=dados.get('acervo'), 
                               leitores=dados.get('leitores'), 
                               emprestimos=dados.get('emprestimos'))
    else:
        return render_template("index.html", error=dados.get('error'))
...
def registerBorrow():
    dados = registrarEmprestimo(request)
    if dados.get('leitorExiste') == False:
        return render_template("emprestimos.html", error=dados.get('error'))
    
    elif dados.get('livroExiste') == False:
        return render_template("emprestimos.html", error=dados.get('error'))
    
    elif dados.get('emprestimo') == False:
        return render_template("emprestimos.html", error=dados.get('error'))
    
    elif dados.get('emprestimos') == True:
        return render_template("emprestimos.html", message=dados.get('message'))
...
@app.route("/delete_leitor/<int:leitor_id>", methods=["DELETE"])
def deleteLeitor(leitor_id):
    
    conn = get_connection()
    csr = conn.cursor()

    csr.execute( "DELETE FROM leitores WHERE id = %s", (leitor_id,))
    conn.commit()
    csr.close()
    conn.close()
...
#Definição da rota para atualizar informações de um leitor
# @app.route("/edit_leitor/<int:leitor_id>", methods=["GET", "POST"])
# def updateBook(leitor_id):
...
#         return render_template("close.html")
#     else:
#         csr.execute("SELECT * FROM leitores WHERE id = %s", (leitor_id,))
#         infos = csr.fetchone()
#         author = infos[2]
#         csr.close()
#         conn.close()
